#+title: =anki-noter=: An AI-powered Anki notes generator
...
#+texinfo_dir_desc: AI-powered Anki notes generator
...
The single entry point is the =M-x anki-noter= command, which opens a transient menu where you configure the source, target, and generation options before invoking the generate action ([[#h:the-transient-menu][The transient menu]]).
...
The development repository is on [[https://github.com/benthamite/anki-noter][GitHub]].
...
#+vindex: anki-noter-cite-sources
#+vindex: anki-noter-cite-format

The user option ~anki-noter-cite-sources~ controls whether a source citation is appended to the back of each generated card.  When ~nil~ (the default), no citation is added.  Set this to ~t~ to enable citations.  This default can be toggled per invocation via the =s= key in the transient menu ([[#h:the-transient-menu][The transient menu]]).
...
** AI model
:PROPERTIES:
:CUSTOM_ID: h:ai-model-options
:END:

#+vindex: anki-noter-backend
#+vindex: anki-noter-model
...
Both options can be changed per session via the =-m= key in the transient menu ([[#h:the-transient-menu][The transient menu]]), which sets ~anki-noter-model~ for the duration of the Emacs session.
...
** The transient menu
:PROPERTIES:
:CUSTOM_ID: h:the-transient-menu
:END:
...
- =f= (*File*) --- set the source file.  Prompts with ~read-file-name~.  PDF files are handled transparently: on backends with native PDF support (Anthropic, Google), the file is added to the =gptel= context directly; on other backends, text is extracted via the fallback command ([[#h:pdf-handling-options][PDF handling]]).
...
- =-m= (*AI model*) --- set the =gptel= model for this session.  Displays the current value of ~anki-noter-model~; when unset, the global ~gptel-model~ is used.  See [[#h:ai-model-options][AI model]].
...
1. If a *file* is set, that file is used.  PDF files are handled transparently depending on the backend ([[#h:pdf-handling-options][PDF handling]]).
